	...
	if ( ! function_exists('APBD_get_floating_li'))
	{
		...
		function APBD_get_floating_li($currency,$_default_mc_cur,$currencySymbol,$current_url,$plugin_file,$isReturn=false){
            ...
            <li class="<?php echo esc_attr(!empty($_default_mc_cur->code)&& $currency->code==$_default_mc_cur->code?" active ":"");?>">
                <a href="<?php echo esc_url($current_url); ?>?_amc-currency=<?php echo esc_attr($currency->code);?>"><span class="icon"><img src="<?php echo plugins_url("images/flags/circle/".strtoupper(esc_attr($currency->code)).'.png',$plugin_file); ?>" alt=""></span><span class="text"><?php echo esc_attr($currency->code);?>(<?php echo esc_attr($currencySymbol); ?>)</span></a>
            </li>
		    <?php
            if($isReturn){
                return ob_get_clean();
