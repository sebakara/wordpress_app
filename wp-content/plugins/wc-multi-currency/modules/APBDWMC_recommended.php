<?php
	...
	class APBDWMC_recommended extends AppsBDLiteModule{
		...
		function getButtonInstallLinkHtml($slug,$plugin_path,$pro_version_paths=[]) {
			...
			<button  class="btn  <?php echo esc_attr($pluginObject->cssClass); ?> btn-sm" disabled ><?php echo esc_attr($pluginObject->title); ?></button>
			<?php }else{?>
			<a href="<?php echo esc_url($pluginObject->url); ?>" target="_blank" class="btn  <?php echo esc_attr($pluginObject->cssClass); ?> btn-sm" <?php echo !empty($pluginObject->isDisabled)?"disabled":""; ?> ><?php echo esc_attr($pluginObject->title); ?></a>
			<?php }
			return ob_get_clean();
		}
