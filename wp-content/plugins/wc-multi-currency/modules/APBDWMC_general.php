	...
	class APBDWMC_general extends AppsBDLiteModule{
		...
		function initialize() {
			...
			if(isset($_GET['_amc-currency'])) {
			    $setCurrency=strtoupper(sanitize_text_field($_GET['_amc-currency']));
				if ( $this->hasCurrencyItem( $setCurrency)) {
					APBDWMC_session::SetSession("_mc_active",$setCurrency);
				}
			}
		...
		function OnInit() {
            ...
            add_filter( 'wc_get_price_decimals', [$this,'_wc_get_price_decimals'], 10, 1 );
        }
		...
		function _wc_get_price_decimals($option)
        {
            //$currency_symbol, $currency
            if ( ! empty( $this->active_currency ) ) {
                return absint($this->active_currency->dec_num);
